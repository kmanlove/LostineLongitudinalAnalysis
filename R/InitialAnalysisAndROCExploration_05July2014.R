...
data <- read.csv("~/work/Kezia/Research/EcologyPapers/LostineLongitudinalStudy/Data/LongSamplingData_05July2014.csv", header = T, sep = "\t")
head(data)
...
#--------------------------------------------------------------------------------#
data$new.date <- strptime(data$CaptureDate, forma = "%m/%d/%Y")
...
text(x = 0.3, y = 3.5, "Consistently \n negative")
text(x = 0.9, y = 3.5, "Consistently \n positive")

#-- classify animals in determination cohort into carriage states --#
#-- place cut line at .5: anybody above .5 is chronic --#
...
#--------------------------------------------------------------------------#
#-- Stripplot of ewes' statuses on different testing events through time --#
#--------------------------------------------------------------------------#
data$NumericCaptureDate <- as.POSIXlt(strptime(as.character(data$CaptureDate), format = "%m/%d/%Y"))
plot(x = 0, y = 0, cex = 0, ylim = c(0, length(levels(factor(data$Animal_ID)))), xlim = range(data$NumericCaptureDate))
for(i in 1:length(levels(factor(data$Animal_ID)))){
  points(x = data$NumericCaptureDate, y = levels(factor(data$Animal_ID))[i])
}
...
#-- 1) How does ELISA scale PCR status for BHS?
...
pcr.data <- read.csv("~/work/Kezia/Research/EcologyPapers/LostineLongitudinalStudy/Data/DuBoisDomesticMoviSampling_23Jan2015.csv", header = T)
