...
# use the posterior survival probs from Manlove et al. 2016 Ecology
...
# 27 x 27 cohort matrix, since there are 27 possible birth cohorts for animals in this study.
...
animals <- start.cohort <- last.cohort <- omit <- most.likely.cohort <- known.age <- rep(NA, length(levels(samples$id)))
cohort.dist <- matrix(0, nrow = length(animals), ncol = 27)
years.list <- vector("list", length(animals))
for(i in 1:length(animals)){
  k <- subset(samples, id == levels(samples$id)[i])
  studysheep <- subset(studysheep.in, as.character(Animal_ID) == levels(samples$id)[i])
  animals[i] <- as.character(levels(samples$id)[i])
...
samples$MostLikelyAge <- rep(NA, dim(samples)[1])
for(i in 1:dim(samples)[1]){
  k <- subset(animal.data, as.character(Animal.ID) == as.character(samples$id)[i])
  samples$MostLikelyAge[i] <- as.numeric(as.character(samples$cap_bioyr[i])) - as.numeric(as.character(k$MostLikelyCohort[1]))
  print(i)
}

# cut samples down to just animals that weren't omitted
samples <- subset(samples, id %in% levels(factor(animal.data$Animal.ID)))
...
samples <- subset(samples, movi_qpcr != "Indeterminate")
