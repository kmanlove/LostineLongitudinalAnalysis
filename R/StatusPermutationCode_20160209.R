...
#full.data$capture_date <- full.data$Capture.Date
...
carrier1 <- subset(data, carriage_twoPosOneYear == "carrier")
...
legend("topleft", leg.text, fill = c("grey60", "red"), bty = "n", cex = .9)
abline(v = orig.mean.length.detected, lwd = 2, col = "red")
abline(v = orig.mean.length.detected.carrier1, lwd = 2, col = "red", lty = 2)
abline(v = orig.mean.length.detected.carrier2, lwd = 2, col = "red", lty = 3)
leg.text.2 <- c("Any + tests", "Two + in one year", "+ in two consec years")
